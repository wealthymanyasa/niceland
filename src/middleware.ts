...
export default function middleware(request: NextRequest) {
  ...
  const publicRoutes = ['/auth/signin', '/auth/signup', '/auth/forgot-password', '/auth/admin-signin']
  
  // API routes that don't need protection
  const apiRoutes = ['/api/chat', '/api/products', '/api/auth', '/api/admin']
  ...
  try {
    ...
    if (pathname.startsWith('/admin') && (!user || user.role !== 'admin')) {
      return NextResponse.redirect(new URL('/auth/admin-signin?error=unauthorized', request.url))
    }
    ...
    return NextResponse.redirect(new URL('/auth/admin-signin?error=server_error', request.url))
  }
}
